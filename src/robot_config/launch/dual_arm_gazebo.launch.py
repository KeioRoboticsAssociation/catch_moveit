...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "use_sim_time",
            default_value="true", # Gazebo uses sim time
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "world",
            default_value=PathJoinSubstitution(
                [FindPackageShare("robot_config"), "worlds", "field.world"]
            ),
        ...
        parameters=[{
            "robot_description": Command([
                ...
                PathJoinSubstitution([
                    FindPackageShare("robot_config"), "config", "dual_arm_gazebo.urdf.xacro"
                ]),
                " ",
    ...
    # Spawn entity in Gazebo (with delay)
    delayed_spawn_entity = TimerAction(
        period=10.0, # 60 seconds delay
        actions=[
            Node(
                package="gazebo_ros",
                executable="spawn_entity.py",
                arguments=["-topic", "robot_description", "-entity", "dual_arm"],
                output="screen",
            )
        ],
    )
    ...
    # ##################################################################
    # ## 1. ros2_control_node を手動で起動 (Gazebo用) ##
    # ##################################################################
    ros2_controllers_path = PathJoinSubstitution(
        [
            FindPackageShare("robot_config"),
            "config",
            "dual_arm_gazebo_controllers.yaml", # Use Gazebo specific controller config
        ]
    )
        ...
        parameters=[
            ...
            {"use_sim_time": use_sim_time},
            {
                "moveit_servo": {
                    ...
                    "joint_topic": "/joint_states", # Use joint_states from controller manager
